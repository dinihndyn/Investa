        ...
        <div id={"dropdown" + name} className="z-10 hidden bg-white shadow w-64 dark:bg-gray-700">
          <ul className="text-sm text-gray-700 dark:text-gray-200 border" aria-labelledby={"dropdownMenu" + name}>
            {submenu.map((item, index) => {
              return (
                <li key={index} className={isActiveSubMenu ? 'bg-investa-primary-10 border-investa-primary-70 p-3 ps-10 border-r-4' : 'p-3 ps-10 hover:border-investa-primary-70 hover:bg-investa-primary-10 border-r-4'}>
                  <NavLink
                    key={index}
                    to={item.link}
                    className={({ isActive }) => { setIsActiveSubMenu(isActive) }}
                  >
